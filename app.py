...
# Mapeo de variables del modelo hacia los nombres exactos de la normativa para poder evaluarlos
MAPEO_ML_A_NORMATIVA = {
    'pH': 'pH',
    'CE': 'Conductividad',
    'T': 'Temperatura',
    'OD': 'Oxígeno Disuelto',
...
nombres_modelo_ui = {
    ...
    'CT': 'Coliformes Termotolerantes (NMP/100 mL)',
...
# =========================================================================
# FUNCIONES AUXILIARES PARA LÓGICA DE ECA Y RENDERIZADO
# =========================================================================
...
def obtener_categoria_eca(parametro, valor_ingresado):
    info = NORMATIVA_COMPLETA.get(parametro)
    if not info:
        return "No definido", "N/A"
    
    # Revisar si todas las categorías ECA están en None
    if info['eca_a1'] is None and info['eca_a2'] is None and info['eca_a3'] is None:
        return "Sin valor en normativa", "N/A"

    invertido = info.get('invertido', False)
    categorias = [('A1', info['eca_a1']), ('A2', info['eca_a2']), ('A3', info['eca_a3'])]
    
    for cat_name, lim in categorias:
        if lim is None:
            continue
        
        # Comparación
        if isinstance(lim, tuple):
            if lim[0] <= valor_ingresado <= lim[1]:
                return cat_name, f"{lim[0]} - {lim[1]}"
        elif invertido:
            if valor_ingresado >= lim:
                return cat_name, str(lim)
        else:
            if valor_ingresado <= lim:
                return cat_name, str(lim)
                
    return "EXCEDE A3", "Supera límites"
...
tab1, tab2, tab3 = st.tabs(["📊 Parámetros (modelo ML)", "⚖️ Parámetros (normativa)", "🔬 Resultados"])

# --- TAB 1: Inputs IA (PRINCIPAL) ---
with tab1:
    st.subheader("Parámetros (modelo ML)")
    st.write("👋 ¡Bienvenido! Por favor introduzca los parámetros fisicoquímicos base para que nuestro modelo entrenado evalúe la potabilidad del agua.")
...
# --- TAB 2: Normativa Completa (SECUNDARIO) ---
with tab2:
    st.subheader("Parámetros (normativa)")
    st.write("Introduzca parámetros adicionales para evaluar directamente contra los estándares MINSA y niveles ECA.")
    
    categorias = ["1. Microbiológicos", "2. Organolépticos", "3. Inorgánicos", "4. Orgánicos", "5. Radiactivos"]
    ya_incluidos = set(MAPEO_ML_A_NORMATIVA.values())
...
with tab3:
    ...
    if st.button("🚀 Ejecutar Diagnóstico", use_container_width=True):
        ...
        # 1. SECCIÓN DEL MODELO (PRINCIPAL)
        ...
        if modelo_pipeline:
            ...
            try:
                ...
                st.metric("Confianza de Potabilidad (Modelo ML)", f"{prob*100:.2f}%")
                
                # Identificar parámetros del modelo problemáticos cruzándolos con la normativa
                for param_ml in active_model_keys:
                    val = st.session_state[f"val_{param_ml}"]
                    norm_key = MAPEO_ML_A_NORMATIVA.get(param_ml)
                    
                    if norm_key and norm_key in NORMATIVA_COMPLETA:
                        info = NORMATIVA_COMPLETA[norm_key]
                        limite = info['minsa']
                        
                        if limite is not None:
                            if isinstance(limite, tuple):
                                if not (limite[0] <= val <= limite[1]): 
                                    problematicos_ml.append((param_ml, norm_key, val))
                ...
                if prob > 0.75:
                    st.success("✅ El modelo predice que el agua es APTA para consumo (umbral > 0.75).")
                else:
                    st.error("❌ El modelo predice que el agua NO ES APTA para consumo.")
                    
            except Exception as e:
                st.warning(f"No se pudo ejecutar la inferencia del modelo. Error detallado: {e}")
        else:
            st.info("Modelo no cargado. Verifica la conexión a Hugging Face.")
            
        # Si el agua presenta parámetros problemáticos según el modelo, evaluar tratamiento (ECA)
        if problematicos_ml:
            st.markdown("#### 🚨 Tratamiento Sugerido para Parámetros Problemáticos (ECA)")
            st.write("Los siguientes parámetros ingresados en el modelo afectan la potabilidad y requieren tratamiento según los niveles ECA:")
            
            eca_data = []
            peor_categoria = "A1"
            orden_eca = {"A1": 1, "A2": 2, "A3": 3, "EXCEDE A3": 4, "Sin valor en normativa": 0}
            
            for k_ml, norm_key, val in problematicos_ml:
                cat_asignada, lim_eca = obtener_categoria_eca(norm_key, val)
                eca_data.append({
                    "Parámetro": nombres_modelo_ui[k_ml],
                    "Valor colocado por el usuario": val,
                    "Valor del ECA": lim_eca,
                    "Categoría asignada": cat_asignada
                })
                
                if orden_eca.get(cat_asignada, 0) > orden_eca.get(peor_categoria, 0):
                    peor_categoria = cat_asignada
            
            df_eca = pd.DataFrame(eca_data)
            st.table(df_eca)
            
            st.write(f"**Nivel de Tratamiento Sugerido:** `{peor_categoria}`")
            if peor_categoria == "A1":
                st.info("🟢 **A1:** Requiere desinfección simple (cloración, ozonización).")
            elif peor_categoria == "A2":
                st.warning("🟠 **A2:** Requiere tratamiento convencional (coagulación, floculación, sedimentación, filtración y desinfección).")
            elif peor_categoria == "A3":
                st.error("🔴 **A3:** Requiere tratamiento avanzado (ósmosis inversa, carbón activado, oxidación avanzada).")
            elif peor_categoria == "EXCEDE A3":
                st.error("⚫ **EXCEDE A3:** El agua excede los parámetros para tratamiento convencional/avanzado. No apta para potabilización directa.")

        st.markdown("---")

        # 2. SECCIÓN DE NORMATIVA (SECUNDARIA)
        st.markdown("### 🏛️ Evaluación: Parámetros Normativos")
        
        if not active_norm_keys:
            st.info("ℹ️ No se activaron parámetros en el apartado *Parámetros (normativa)* para evaluación adicional.")
        else:
            st.write(f"Evaluando **{len(active_norm_keys)}** parámetros adicionales contra MINSA:")
            incumplimientos_minsa = []
            
            for param in active_norm_keys:
                valor = st.session_state[f"val_{param}"]
                info = NORMATIVA_COMPLETA[param]
                minsa_lim = info['minsa']
                
                if minsa_lim is None:
                    incumplimientos_minsa.append({
                        "Parámetro": param, 
                        "Valor colocado por el usuario": valor, 
                        "Límite Normativo (MINSA)": "Sin valor en normativa",
                        "Estado": "No evaluable"
                    })
                    continue
                ...
                if isinstance(minsa_lim, tuple):
                    if not (minsa_lim[0] <= valor <= minsa_lim[1]):
                        incumplimientos_minsa.append({
                            "Parámetro": param, 
                            "Valor colocado por el usuario": valor, 
                            "Límite Normativo (MINSA)": f"{minsa_lim[0]} - {minsa_lim[1]}",
                            "Estado": "EXCEDE (Fuera de rango)"
                        })
                elif info.get('invertido', False):
                    if valor < minsa_lim:
                        incumplimientos_minsa.append({
                            "Parámetro": param, 
                            "Valor colocado por el usuario": valor, 
                            "Límite Normativo (MINSA)": f">= {minsa_lim}",
                            "Estado": "EXCEDE (Menor al mínimo)"
                        })
                else:
                    if valor > minsa_lim:
                        incumplimientos_minsa.append({
                            ...
                            "Valor colocado por el usuario": valor, 
                            "Límite Normativo (MINSA)": f"<= {minsa_lim}",
                            "Estado": "EXCEDE (Mayor al máximo)"
                        })
            
            excedentes = [x for x in incumplimientos_minsa if x["Estado"] != "No evaluable"]
            
            if excedentes:
                st.error(f"⚠️ {len(excedentes)} parámetros adicionales sobrepasan los límites MINSA:")
            else:
                st.success("✅ Los parámetros evaluados cumplen con la normativa MINSA (o no poseen límites aplicables).")
